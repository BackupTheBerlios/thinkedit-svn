...
class node
{
		...
		**/
		function node($table = 'node')
		{
				...
				global $node_list_cache; // uggly, but does the job
				
				if ($node_list_cache)
				{
						$this->node_list = $node_list_cache;
				}
				else
				{
						$records = $this->record->find();
						$node_list_cache = $records;
						$this->node_list = $node_list_cache;
		...
		**/
		...
		{
				global $thinkedit;
				foreach ($this->node_list as $record)
				{
						if ($record->get('id') == $this->record->get('parent_id'))
						{
								$parent_node = $thinkedit->newNode($this->table, $record->get('id'));
								return $parent_node;
						}
