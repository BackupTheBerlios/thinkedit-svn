...
class node
{
	...
	**/
	function delete($keep_content = true)
	{
		...
		else
		{
			...
			global $thinkedit;
			$thinkedit->event->trigger('node_before_delete', $this);
			...
			$this->rebuild();
			$thinkedit->event->trigger('node_after_delete', $this);
			return $result;
		}
	}
	...
	**/
	function save()
	{
		...
		//$this->record->load();
		global $thinkedit;
		$thinkedit->event->trigger('node_before_save', $this);
	...
	*/
	...
	{
		global $thinkedit;
		$thinkedit->event->trigger('node_before_add', $this);	
			...
			if ($where=='top')
			{
				...
				*/
				...
				if ($results)
				{
					...
					$node->is_new = true;
					$thinkedit->event->trigger('node_after_add', $this);
					return $node;
				}
				else
				{
