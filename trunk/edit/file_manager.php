...
{
  $path = $_REQUEST['path'];
  $_SESSION[$module]['path'] = $path;
}
elseif ($_SESSION[$module]['path'])
{
  $path = $_SESSION[$module]['path'];
}
else
{
  $path = '/';
...
if ($sync)
{
  ...
  $sql_files = $db->get_results("select filename from $table where path='$path' group by 'filename', 'path' order by filename");
  ...
  if (!$ftp_files_list) $ftp_files_list[0] = "empty folder";
  ...
  if (!$sql_files_list) $sql_files_list[0] = "empty folder";
  ...
  /*
  Phase II, handling of folders
  */



  $ftp_folders = $server->ls('',NET_FTP_DIRS_ONLY);

  $sql_folders = $db->get_results("select distinct path from $table");


	//modify sql_folders array
	
	foreach ($sql_folders as $sql_folder)
	{
	$sql_folders_ok[] = $sql_folder->path;
	}

  if ($debug) print_a($ftp_folders);
  if ($debug) print_a($sql_folders_ok);



  foreach ($ftp_folders as $ftp_folder)
  {
    $ftp_path = $path . $ftp_folder['name'];

		// si un ftp ne se trouve pas dans le sql, l'ajouter

    if (!in_array($ftp_path, $sql_folders_ok))
    {
      if ($debug) echo $ftp_path . ' not in sql';
			$db->query("insert into $table (filename, path) values ('', '$ftp_path')");
